...
for tweet in tweepy.Paginator(client.search_all_tweets, query=query, start_time=start_time, end_time=end_time,
    ...
    #client.get_tweets(ids=tweet.ids, tweet_fields=['context_annotations','created_at','geo'])
    
    #filehandle.write('%s\n' % tweet.text)
    df.append(tweet.text)
    df_time.append(tweet.created_at)
...
#dfnew.to_csv("tweet11.csv")


#!curl --request GET 'https://api.twitter.com/2/tweets/search/all?query=unemployment&start_time=2020-01-01T00:00:00.00Z&end_time=2020-01-01T00:00:00.00Z&place_country:US' --header 'Authorization: Bearer AAAAAAAAAAAAAAAAAAAAALRkaQEAAAAA989GenO8QF9h9yCLf%2Brw9882rQA%3DzwzX620NkZZfvdE0N0I0yzEIDWVdRISuZSLx48MER6hY66HlnL'
